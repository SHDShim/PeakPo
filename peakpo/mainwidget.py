...
#        self.chi_path       = '' #reduendant, but easier to code
        ...
        self.mpldoubleSpinBox_Pressure.valueChanged.connect(self.set_pressure)
        self.mpldoubleSpinBox_Pressure.setKeyboardTracking(False)
        self.mpldoubleSpinBox_Temperature.setValue(self.temperature)
        self.mpldoubleSpinBox_Temperature.valueChanged.connect(self.set_temp)
...
#        self.mpltableWidget_UnitCell.resizeRowsToContents()
    ...
    def save_XLS(self):
        ...
        if str(filen_xls) == '':
            return
        xls_jlist(filen_xls, self.jlist, self.pressure, self.temperature)
...
#               Make sure you overwrite the file again after opening.')
        ...
        if jlistonly:
            ...
            self.pressure = session.pressure
    ...
    def _dump_session(self, fsession):
        ...
        session.wavelength = self.wavelength
        session.pressure = self.pressure
    ...
    def add_patterns(self):
        ...
        for f in files:
            ...
            pattern.wavelength = self.wavelength
...
#        n_files = files.__len__()
    ...
    def _find_closestjcpds(self, x):
        ...
        if jcount != 0:
            ...
            for j in self.jlist:
                if j.display:
                    i, d, t = j.find_DiffLine(x, self.wavelength)
                    idx_j.append(i)
                    diff_j.append(d)
                    tth_j.append(t)
        ...
        if ucount != 0:
            ...
            for u in self.ucfitlist:
                if u.display:
                    i, d, t = u.find_DiffLine(x, self.wavelength)
                    idx_u.append(i)
                    diff_u.append(d)
                    tth_u.append(t)
...
#            self.update_graph()
    ...
    def _read_a_newpattern(self, new_filename):
        ...
        self.Pattern.wavelength = self.wavelength
    ...
    def update_graph(self, limits=None):
        ...
        xlabel = 'Two Theta (Degrees), ' + \
            ...
            format(self.pressure, self.temperature, self.wavelength)
        ...
        print("Total plot takes: %.4f second" % (time.time() - t_start))
        self.unsetCursor()
    ...
    def _plot_ucfit(self):
        ...
        for j in self.ucfitlist:
            if j.display:
                j.cal_dsp()
                tth, inten = j.get_tthVSint(self.wavelength)
    ...
    def _plot_jcpds(self):
        ...
        for j in self.jlist:
            if j.display:
                j.cal_dsp(self.pressure, self.temperature)
                tth, inten = j.get_tthVSint(self.wavelength)
